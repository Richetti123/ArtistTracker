...
function profileImage(context) {
  return context?.profile?.imageUrl || context?.profile?.spotifyImageUrl || '';
}
...
export async function sendArtistEvidence(sock, artist, rawSendMessage, destination = config.targetJid, context = null, sendOverride = null) {
  ...
  if (profileUrl) {
    try {
      await send(chat, { image: { url: profileUrl }, caption: `🎤 *${clean}*\n📸 Foto de perfil del artista encontrada en Spotify.` });
      console.log('  │  ├─ 🎤 Foto de perfil del artista enviada.');
    } catch (err) {
      console.error(`  │  └─ ⚠️ No se pudo enviar la foto de perfil: ${err.message}`);
    }
  }
  ...
  const lines = [
    ...
    visual.length ? `🧠 *Visión IA:* ${visual.map(x => `${x.landmark || 'posible ubicación en Madrid'} (${Math.round(Math.max(x.confidence || 0, x.landmarkConfidence || 0) * 100)}%)`).join(', ')}` : '🧠 *Visión IA:* no se detectó una imagen que fuera evidencia geográfica fiable; las portadas de canciones/álbumes no se adjuntan como prueba.'
  ];
