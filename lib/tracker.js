...
async function artistProfile(artist, hint = '') {
  ...
  try {
    ...
    return {
      country: String(parsed.country || hint || 'No identificado').trim(),
...
async function analyzeArtist(artist, countryHint = '', { save = false, send = false, sock = null } = {}) {
  ...
  if (Number.isFinite(listeners.monthly)) {
    console.log(`  ├─ 🎧 Oyentes: ${listeners.monthly.toLocaleString('es-ES')} (${listeners.source})`);
  } else {
    console.log('  ├─ ❌ No se pudieron verificar los oyentes mensuales.');
  }
  ...
  if (send && sock) {
    const listenerText = Number.isFinite(listeners.monthly)
      ? `${listeners.monthly.toLocaleString('es-ES')} (${listeners.source})`
      : 'No verificados';
    ...
    await sock.sendMessage(config.targetJid, { text: response });
  }
...
export async function manualSearch(sock, artist) {
  const clean = String(artist || '').trim();
  if (!clean) return false;
  await sock.sendMessage(config.targetJid, { text: `🔎 Voy a comprobar manualmente *${clean}* en ArtistTracker. Revisaré país, género, oyentes y las fuentes sociales para intentar confirmar si existe actividad actual en Madrid.` });
  try {
    await analyzeArtist(clean, '', { send: true, sock });
  } catch (err) {
    console.error('[MANUAL] Error:', err);
    await sock.sendMessage(config.targetJid, { text: `❌ No pude completar la búsqueda manual de *${clean}*. Revisa la consola para ver el error.` });
  }
  return true;
}
